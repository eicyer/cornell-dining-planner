...
**Security hardening** (`docs/adr/0018-session-cookie-and-csrf-hardening.md`): fail-fast production config, session-cookie/CSRF hardening, env-driven CORS + `TrustedHostMiddleware`, CSP/security-headers middleware (nonce-based, no `unsafe-inline`), rate limiting on the OAuth and LLM-backed routes (plus a per-user daily cap on forced crafted-meal rebuilds — `app.routers.crafted_meals.MAX_BUILDS_PER_DAY`), bounded LLM-tag output, self-service account deletion (`DELETE /auth/me`), a `backend/tests/security/` regression suite, Dependabot, and a hash-pinned backend lockfile (`backend/requirements.in` → `requirements.txt`). One known, accepted-risk gap remains, documented rather than silently left: `frontend/package-lock.json` carries ~16 `npm audit` findings (0 critical) confined to Expo/Metro's build tooling (image parsing, dev-server deps), whose only fixes are a semver-major Expo/React Native downgrade — left for Dependabot to propose as a reviewed PR rather than force-applied here. (`starlette`'s CVEs, previously the other accepted-risk gap here, were resolved by the `fastapi==0.141.1` upgrade — see `backend/requirements.in`.)
